...
class StatefulTemplateManager():        
    ...
    def from_dict(cls, json_data):
        ...
        for state in templates:
            if json_data.get(state) != None:
                for template_type in templates.get(state):
                    if json_data.get(state).get(template_type) != None:
...
class InventoryNotFound(Error):
    def __init__(self, inventory_name):
        self.msg = 'No ID was found for the provided inventory name "{0}". Verify the inventory exists and that provided capitalization and punctuation is correct.'.format(inventory_name)
        
class VarsNotFound(Error):
    def __init__(self, inventory_name, inventory_id):
        self.msg = 'No vars were found for the provided inventory name "{0}" and ID "{1}". Verify the inventory exists and provided capitalization and punctuation is correct.'.format(inventory_name, str(inventory_id))
