...
class Workflow(Template):
    ...
    def can_copy(self):
        ...
        if res == None:
            return False
        if res.get('can_copy') == True and res.get('can_copy_without_user_input') == True:
            return True
        return False
...
class WorkflowNotFound(Error):
    def __init__(self, workflow_name):
        self.msg = 'No ID was found for the provided workflow job template name "{0}". Verify the workflow exists and that provided capitalization and punctuation is correct.'.format(workflow_name)
...
class UserNotFound(Error):
    def __init__(self, field):
        self.msg = 'No ID/username was found for the provided corresponding ID/username "{0}". Verify the username exists and that provided capitalization and punctuation is correct.'.format(str(field))
class RoleNotFound(Error):
    def __init__(self, role, role_container, role_container_id):
        self.msg = 'No ID was found for the role "{0}" in "{1}" with ID "{2}". Verify the role exists and that provided capitalization and punctuation is correct.'.format(role, role_container, str(role_container_id))
class RoleUpdateUserFail(Error):
    def __init__(self, username, user_id, role, role_container, role_container_id):
        self.msg = 'An attempt to add username "{0}" with ID "{1}" to role "{2}" in "{3}" with ID "{4}" failed.'.format(username, str(user_id), role, role_container, str(role_container_id)) 
...
def check_for_missing_args(module):
    ...
    if expected_state == 'present' or expected_state == 'copied':
        if module.params.get('username') == None and module.params.get('user_id') == None:
            raise UserParamMissing()
        if module.params.get('provisioned_instance_name') == None:
            raise MissingArg('provisioned_instance_name', 'present/copied')
    else:
        if module.params.get('workflow_name') == None:
...
def run_module():
    ...
    parameter_defaults = {
        'description': 'Workflow referencing provisioned instance {0} for username {1}'.format(module.params.get('provisioned_instance_name'), module.params.get('username'))
    }
